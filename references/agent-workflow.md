...
## 11. Render-Level Front Matter Rules

Except for the geometric cover page, front matter must be rendered with Word-native flow: paragraphs, paragraph styles, tab stops, section/page breaks, headers/footers, page-number fields, and a Word TOC field. Do not use `w:tbl` to align the spine, task book, declaration, Chinese abstract, English abstract, TOC, or copied cover metadata. Run `python scripts/validate_layout_consistency.py --reference <reference.docx> --candidate output/thesis.docx --sample-mode truncated --out output/layout_consistency_report.json` after generating a debug sample. The report must at least pass `non_cover_tables`, `toc_field`, `roman_front_matter_page_numbers`, `body_decimal_page_restart`, and `forbidden_body_text`.

PDF hard line breaks must be normalized before rendering. Use `buaa_thesis_kit.text_flow.merge_pdf_lines` / `merge_text` for extracted abstract and body text so Chinese lines merge without inserted spaces, English hyphenation is repaired, and headers, page numbers, and cover field labels are not merged into body paragraphs. Cover metadata tables extracted from source DOCX/PDF must be filtered from body tables; the authoritative cover is filled from the template instead.
