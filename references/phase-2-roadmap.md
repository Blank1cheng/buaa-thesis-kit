...
Phase 1 now supports DOCX and PDF inputs, layout-aware PDF text-line extraction, clean output packaging, Word-authoritative PDF export, editable Word auditing, fixed front-matter rendering, and conservative review reporting. Phase 1.1 also includes strict finalization mode through `--strict`, where unresolved review items produce `strict_finalization_failed`. Phase 2 should harden the remaining boundary cases:
...
- Golden front-matter visual fixtures that compare the fixed renderer against reference DOCX/PDF page geometry for cover logo positions, title line breaks, vertical spine placement, abstract headers, TOC page numbers, and body page-number restart.
