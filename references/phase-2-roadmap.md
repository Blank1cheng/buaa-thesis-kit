...
Phase 1 now supports DOCX and PDF inputs, layout-aware PDF text-line extraction, clean output packaging, Word-authoritative PDF export, editable Word auditing, render-first front-matter scaffolding, and conservative review reporting. Phase 1.1 also includes strict finalization mode through `--strict`, where unresolved review items produce `strict_finalization_failed`. Phase 2 should harden the remaining boundary cases:
...
- Golden front-matter visual fixtures that compare `frontmatter_render` output against reference DOCX/PDF page geometry for cover logo positions, title line breaks, vertical spine placement, task book I/II/III/IV blocks, declaration signature fields, abstract headers, TOC page numbers, and body page-number restart. Use `templates/frontmatter/layout_spec.yaml` as the editable anchor map and `buaa_thesis_kit/styles/buaa_styles.py` as the central style registry before changing renderer constants.
