...
6. 任一字符无法由独立证据消歧时，将同一公式记录为 `needs_review`，保留候选、裁剪、来源和缺失证据；不得把猜测写入 `thesis.tex`，并阻止总体状态成为 `pass`。允许交付 `status=needs_review` 的完整复核包，供人工基于同一稳定公式 ID 闭环，但不得把该复核包宣称为通过或最终定稿。
