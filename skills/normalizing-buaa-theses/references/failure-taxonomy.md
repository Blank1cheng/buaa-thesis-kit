...
每个未闭环问题写入 `failure_queue.json`，ID 采用 `H-<gate>-<序号>` 并在复修过程中保持不变。记录必须含：
...
evidence:
  ...
  bbox: [x0, y0, x1, y1]
  hash: source-or-artifact-sha256
...
`evidence` 与 `evidence_text` 必须同时存在，不能二选一。结构化 `evidence` 用 `paths` 定位源文件、截图或裁剪，用 `page` 和 `bbox` 锁定区域，并用 `hash` 绑定证据版本；`evidence_text` 提供与旧消费者兼容的简短事实摘要，不替代结构化定位。
